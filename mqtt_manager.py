...
class MqttManager:
    ...
    def agent_sensor_data(self, client, userdata, msg ) -> None:
        ...
        try:
        ...
        except StopIteration: #IS DRONE OPERATOR
            json_msg = json.loads(str_msg) #ValueError
            ...
            try:
                if agent_attri == "response":
                    task: Task = next(( t for t in self.agent_manager.running_tasks if t.task_uuid == json_msg["task-uuid"] )) #StopIteration
                    json_msg["fail-reason"] = task.agent.meta["name"]
                    self.agent_manager.check_response(self.client, f"{self.unique_ussp_topic}/command", self.ussp_event, json_msg, task)
                    self.send_response(json_msg)
                elif agent_attri == "feedback":
                    task: Task = next(( t for t in self.agent_manager.running_tasks if t.task_uuid == json_msg["task-uuid"] )) #StopIteration
                    self.agent_manager.check_feedback(self.client, f"{self.unique_ussp_topic}/command", self.ussp_event, json_msg, task)
            ...
            if command == AgentCommand.PING:
                ...
                if not self.task_queue.queue.full():
                    ...
                    if self.drone_operator_manager.children:
                        dummy_agent: Agent = Agent({"name": "Drone From Team Member"}) #This name is used in the functions check_feedback & check_response in agent_manager.py
